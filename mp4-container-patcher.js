(() => {
  ...
  function patchKryptonContainer(inputBytes) {
    ...
    const patchedMoovFirstPass = rebuildBox(source, moov, {
      ...
      replacements: makeVideoTableReplacements({
        source,
        stsz,
        stts,
        stsc,
  ...
  function rebuildBox(source, box, context) {
    ...
    if (context.extendEditDuration) patchItzcrihStyleDurations(rebuilt, box.type);
    return rebuilt;
  }

  function patchItzcrihStyleDurations(boxBytes, type) {
    if (type === "mvhd") {
